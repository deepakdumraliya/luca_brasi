...
class Dashboard extends BaseController
{
        ...
        if ($this->request->getPost() != "") {
            ...
            if (!$this->validate($rules)) {
                ...
                $driverdetails = array(
                    ...
                    'password' => trim(md5($this->request->getPost('txtpassword'))),
    ...
    {
        $messages['drivers'] = $this->db->table('message')->select('message.user_id,first_name,last_name,created,message,status')->join('tbl_user', 'tbl_user.user_id=message.user_id')->groupBy('message.user_id')->orderBy('message_id', 'DESC')->get()->getResultArray();
        $messages['drivers_count'] = $this->db->table('message')->select('message.user_id,first_name,last_name,created,message,status,count(*) as message_count')->join('tbl_user', 'tbl_user.user_id=message.user_id')->where('message.status', 0)->groupBy('message.user_id')->get()->getResultArray();
        $this->header();
    ...
    {
        if ($this->request->getPost() != "") {
            $message = $this->db->table('message')->select('message.user_id,created,message,status')->where('user_id', $this->request->getPost('user_id'))->orderBy('message_id', 'DESC')->get()->getResultArray();
            if (!empty($message)) {
                $this->db->table('message')->where('user_id', $this->request->getPost('user_id'))->update(array('status' => 1));
            }
                    ...
                    <div class="received_msg">
                        <div class="received_withd_msg">
                            <p><?php echo $msg['message'];
                                ?></p>
                            <span class="time_date"> <?php echo date("H:m", strtotime($msg['created'])); ?> |<?php echo date("F d", strtotime($msg['created'])); ?></span>
                        </div>
                    </div>
                </div>
<?php
